// Copyright 2025 Cisco Systems, Inc. and its affiliates
//
// SPDX-License-Identifier: Apache-2.0

/*
Cisco Secure Access Resource Connector Groups and Resource Connectors API

Manage the properties of the Resource Connector Groups and Resource Connectors in the organization. 

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package resconn

import (
	"encoding/json"
)

// checks if the PatchConnector400Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PatchConnector400Response{}

// PatchConnector400Response struct for PatchConnector400Response
type PatchConnector400Response struct {
	// Error message explaining the reason for failure.
	Error *string `json:"error,omitempty"`
	// The ID of the request.
	RequestId *string `json:"requestId,omitempty"`
	ValidationErrors *PatchConnector400ResponseValidationErrors `json:"validationErrors,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _PatchConnector400Response PatchConnector400Response

// NewPatchConnector400Response instantiates a new PatchConnector400Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPatchConnector400Response() *PatchConnector400Response {
	this := PatchConnector400Response{}
	return &this
}

// NewPatchConnector400ResponseWithDefaults instantiates a new PatchConnector400Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPatchConnector400ResponseWithDefaults() *PatchConnector400Response {
	this := PatchConnector400Response{}
	return &this
}

// GetError returns the Error field value if set, zero value otherwise.
func (o *PatchConnector400Response) GetError() string {
	if o == nil || IsNil(o.Error) {
		var ret string
		return ret
	}
	return *o.Error
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchConnector400Response) GetErrorOk() (*string, bool) {
	if o == nil || IsNil(o.Error) {
		return nil, false
	}
	return o.Error, true
}

// HasError returns a boolean if a field has been set.
func (o *PatchConnector400Response) HasError() bool {
	if o != nil && !IsNil(o.Error) {
		return true
	}

	return false
}

// SetError gets a reference to the given string and assigns it to the Error field.
func (o *PatchConnector400Response) SetError(v string) {
	o.Error = &v
}

// GetRequestId returns the RequestId field value if set, zero value otherwise.
func (o *PatchConnector400Response) GetRequestId() string {
	if o == nil || IsNil(o.RequestId) {
		var ret string
		return ret
	}
	return *o.RequestId
}

// GetRequestIdOk returns a tuple with the RequestId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchConnector400Response) GetRequestIdOk() (*string, bool) {
	if o == nil || IsNil(o.RequestId) {
		return nil, false
	}
	return o.RequestId, true
}

// HasRequestId returns a boolean if a field has been set.
func (o *PatchConnector400Response) HasRequestId() bool {
	if o != nil && !IsNil(o.RequestId) {
		return true
	}

	return false
}

// SetRequestId gets a reference to the given string and assigns it to the RequestId field.
func (o *PatchConnector400Response) SetRequestId(v string) {
	o.RequestId = &v
}

// GetValidationErrors returns the ValidationErrors field value if set, zero value otherwise.
func (o *PatchConnector400Response) GetValidationErrors() PatchConnector400ResponseValidationErrors {
	if o == nil || IsNil(o.ValidationErrors) {
		var ret PatchConnector400ResponseValidationErrors
		return ret
	}
	return *o.ValidationErrors
}

// GetValidationErrorsOk returns a tuple with the ValidationErrors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchConnector400Response) GetValidationErrorsOk() (*PatchConnector400ResponseValidationErrors, bool) {
	if o == nil || IsNil(o.ValidationErrors) {
		return nil, false
	}
	return o.ValidationErrors, true
}

// HasValidationErrors returns a boolean if a field has been set.
func (o *PatchConnector400Response) HasValidationErrors() bool {
	if o != nil && !IsNil(o.ValidationErrors) {
		return true
	}

	return false
}

// SetValidationErrors gets a reference to the given PatchConnector400ResponseValidationErrors and assigns it to the ValidationErrors field.
func (o *PatchConnector400Response) SetValidationErrors(v PatchConnector400ResponseValidationErrors) {
	o.ValidationErrors = &v
}

func (o PatchConnector400Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PatchConnector400Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Error) {
		toSerialize["error"] = o.Error
	}
	if !IsNil(o.RequestId) {
		toSerialize["requestId"] = o.RequestId
	}
	if !IsNil(o.ValidationErrors) {
		toSerialize["validationErrors"] = o.ValidationErrors
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *PatchConnector400Response) UnmarshalJSON(data []byte) (err error) {
	varPatchConnector400Response := _PatchConnector400Response{}

	err = json.Unmarshal(data, &varPatchConnector400Response)

	if err != nil {
		return err
	}

	*o = PatchConnector400Response(varPatchConnector400Response)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "error")
		delete(additionalProperties, "requestId")
		delete(additionalProperties, "validationErrors")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullablePatchConnector400Response struct {
	value *PatchConnector400Response
	isSet bool
}

func (v NullablePatchConnector400Response) Get() *PatchConnector400Response {
	return v.value
}

func (v *NullablePatchConnector400Response) Set(val *PatchConnector400Response) {
	v.value = val
	v.isSet = true
}

func (v NullablePatchConnector400Response) IsSet() bool {
	return v.isSet
}

func (v *NullablePatchConnector400Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePatchConnector400Response(val *PatchConnector400Response) *NullablePatchConnector400Response {
	return &NullablePatchConnector400Response{value: val, isSet: true}
}

func (v NullablePatchConnector400Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePatchConnector400Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


