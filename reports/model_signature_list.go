// Copyright 2025 Cisco Systems, Inc. and its affiliates
//
// SPDX-License-Identifier: Apache-2.0

/*
Cisco Secure Access Reports

Reporting App Discovery API Usage Metering

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package reports

import (
	"encoding/json"
	"fmt"
)

// checks if the SignatureList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SignatureList{}

// SignatureList The properties of the signature list.
type SignatureList struct {
	// The unique ID assigned to a default or custom signature list.
	Id int64 `json:"id"`
	AdditionalProperties map[string]interface{}
}

type _SignatureList SignatureList

// NewSignatureList instantiates a new SignatureList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSignatureList(id int64) *SignatureList {
	this := SignatureList{}
	this.Id = id
	return &this
}

// NewSignatureListWithDefaults instantiates a new SignatureList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSignatureListWithDefaults() *SignatureList {
	this := SignatureList{}
	return &this
}

// GetId returns the Id field value
func (o *SignatureList) GetId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *SignatureList) GetIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *SignatureList) SetId(v int64) {
	o.Id = v
}

func (o SignatureList) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SignatureList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *SignatureList) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSignatureList := _SignatureList{}

	err = json.Unmarshal(data, &varSignatureList)

	if err != nil {
		return err
	}

	*o = SignatureList(varSignatureList)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableSignatureList struct {
	value *SignatureList
	isSet bool
}

func (v NullableSignatureList) Get() *SignatureList {
	return v.value
}

func (v *NullableSignatureList) Set(val *SignatureList) {
	v.value = val
	v.isSet = true
}

func (v NullableSignatureList) IsSet() bool {
	return v.isSet
}

func (v *NullableSignatureList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSignatureList(val *SignatureList) *NullableSignatureList {
	return &NullableSignatureList{value: val, isSet: true}
}

func (v NullableSignatureList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSignatureList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


