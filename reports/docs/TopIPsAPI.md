...
------------- | ------------- | -------------
[**GetTopIps**](TopIPsAPI.md#GetTopIps) | **Get** /reports/v2/top-ips | Get Top IPs
[**GetTopIpsInternal**](TopIPsAPI.md#GetTopIpsInternal) | **Get** /reports/v2/top-ips/internal | Get Top IPs Internal



## GetTopIps

> GetTopIps200Response GetTopIps(ctx).From(from).To(to).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	resp, r, err := apiClient.TopIPsAPI.GetTopIps(context.Background()).From(from).To(to).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopIPsAPI.GetTopIps``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopIps`: GetTopIps200Response
	fmt.Fprintf(os.Stdout, "Response from `TopIPsAPI.GetTopIps`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopIpsRequest struct via the builder pattern
...
[**GetTopIps200Response**](GetTopIps200Response.md)
...
> GetTopIps200Response GetTopIpsInternal(ctx).From(from).To(to).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TopIPsAPI.GetTopIpsInternal(context.Background()).From(from).To(to).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopIPsAPI.GetTopIpsInternal``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopIpsInternal`: GetTopIps200Response
	fmt.Fprintf(os.Stdout, "Response from `TopIPsAPI.GetTopIpsInternal`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopIpsInternalRequest struct via the builder pattern
