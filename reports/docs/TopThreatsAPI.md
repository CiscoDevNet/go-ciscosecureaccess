...
------------- | ------------- | -------------
[**GetTopThreats**](TopThreatsAPI.md#GetTopThreats) | **Get** /reports/v2/top-threats | Get Top Threats (All)
[**GetTopThreatsByType**](TopThreatsAPI.md#GetTopThreatsByType) | **Get** /reports/v2/top-threats/{type} | Get Top Threats (By Type)



## GetTopThreats

> GetTopThreats200Response GetTopThreats(ctx).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	resp, r, err := apiClient.TopThreatsAPI.GetTopThreats(context.Background()).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopThreatsAPI.GetTopThreats``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopThreats`: GetTopThreats200Response
	fmt.Fprintf(os.Stdout, "Response from `TopThreatsAPI.GetTopThreats`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopThreatsRequest struct via the builder pattern
...
[**GetTopThreats200Response**](GetTopThreats200Response.md)
...
> GetTopThreats200Response GetTopThreatsByType(ctx, type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TopThreatsAPI.GetTopThreatsByType(context.Background(), type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Threats(threats).Threattypes(threattypes).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopThreatsAPI.GetTopThreatsByType``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopThreatsByType`: GetTopThreats200Response
	fmt.Fprintf(os.Stdout, "Response from `TopThreatsAPI.GetTopThreatsByType`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopThreatsByTypeRequest struct via the builder pattern
