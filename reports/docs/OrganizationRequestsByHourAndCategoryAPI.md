...
------------- | ------------- | -------------
[**GetRequestsByHourAndCategory**](OrganizationRequestsByHourAndCategoryAPI.md#GetRequestsByHourAndCategory) | **Get** /reports/v2/categories-by-hour | Get Requests by Hour and Category (All)
[**GetRequestsByHourAndCategoryAndType**](OrganizationRequestsByHourAndCategoryAPI.md#GetRequestsByHourAndCategoryAndType) | **Get** /reports/v2/categories-by-hour/{type} | Get Requests by Hour and Category



## GetRequestsByHourAndCategory

> GetRequestsByHourAndCategory200Response GetRequestsByHourAndCategory(ctx).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	resp, r, err := apiClient.OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategory(context.Background()).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategory``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetRequestsByHourAndCategory`: GetRequestsByHourAndCategory200Response
	fmt.Fprintf(os.Stdout, "Response from `OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategory`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetRequestsByHourAndCategoryRequest struct via the builder pattern
...
## GetRequestsByHourAndCategoryAndType

> GetRequestsByHourAndCategory200Response GetRequestsByHourAndCategoryAndType(ctx, type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategoryAndType(context.Background(), type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategoryAndType``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetRequestsByHourAndCategoryAndType`: GetRequestsByHourAndCategory200Response
	fmt.Fprintf(os.Stdout, "Response from `OrganizationRequestsByHourAndCategoryAPI.GetRequestsByHourAndCategoryAndType`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetRequestsByHourAndCategoryAndTypeRequest struct via the builder pattern
