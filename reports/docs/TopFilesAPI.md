...
func main() {
	...
	resp, r, err := apiClient.TopFilesAPI.GetTopFilesProxy(context.Background()).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopFilesAPI.GetTopFilesProxy``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopFilesProxy`: GetTopFilesProxy200Response
	fmt.Fprintf(os.Stdout, "Response from `TopFilesAPI.GetTopFilesProxy`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopFilesProxyRequest struct via the builder pattern
...
[**GetTopFilesProxy200Response**](GetTopFilesProxy200Response.md)
