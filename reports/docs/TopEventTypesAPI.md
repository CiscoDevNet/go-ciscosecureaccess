...
func main() {
	...
	resp, r, err := apiClient.TopEventTypesAPI.GetTopEventTypes(context.Background()).From(from).To(to).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TopEventTypesAPI.GetTopEventTypes``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTopEventTypes`: GetTopEventTypes200Response
	fmt.Fprintf(os.Stdout, "Response from `TopEventTypesAPI.GetTopEventTypes`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTopEventTypesRequest struct via the builder pattern
...
[**GetTopEventTypes200Response**](GetTopEventTypes200Response.md)
