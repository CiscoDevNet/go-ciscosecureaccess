...
func main() {
	...
	resp, r, err := apiClient.IdentityDistributionAPI.GetIdentityDistribution(context.Background()).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `IdentityDistributionAPI.GetIdentityDistribution``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetIdentityDistribution`: GetIdentityDistribution200Response
	fmt.Fprintf(os.Stdout, "Response from `IdentityDistributionAPI.GetIdentityDistribution`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetIdentityDistributionRequest struct via the builder pattern
...
[**GetIdentityDistribution200Response**](GetIdentityDistribution200Response.md)
...
## GetIdentityDistributionType

> GetIdentityDistributionType200Response GetIdentityDistributionType(ctx, type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.IdentityDistributionAPI.GetIdentityDistributionType(context.Background(), type_).From(from).To(to).Limit(limit).Offset(offset).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `IdentityDistributionAPI.GetIdentityDistributionType``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetIdentityDistributionType`: GetIdentityDistributionType200Response
	fmt.Fprintf(os.Stdout, "Response from `IdentityDistributionAPI.GetIdentityDistributionType`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetIdentityDistributionTypeRequest struct via the builder pattern
...
[**GetIdentityDistributionType200Response**](GetIdentityDistributionType200Response.md)
