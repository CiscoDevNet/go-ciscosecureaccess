...
------------- | ------------- | -------------
[**GetTotalRequests**](TotalRequestsAPI.md#GetTotalRequests) | **Get** /reports/v2/total-requests | Get Total Requests (All)
[**GetTotalRequestsByType**](TotalRequestsAPI.md#GetTotalRequestsByType) | **Get** /reports/v2/total-requests/{type} | Get Total Requests (By Type)



## GetTotalRequests

> GetTotalRequests200Response GetTotalRequests(ctx).From(from).To(to).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Ruleid(ruleid).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
...
func main() {
	...
	resp, r, err := apiClient.TotalRequestsAPI.GetTotalRequests(context.Background()).From(from).To(to).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Verdict(verdict).Ruleid(ruleid).Sha256(sha256).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TotalRequestsAPI.GetTotalRequests``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTotalRequests`: GetTotalRequests200Response
	fmt.Fprintf(os.Stdout, "Response from `TotalRequestsAPI.GetTotalRequests`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTotalRequestsRequest struct via the builder pattern
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TotalRequestsAPI.GetTotalRequestsByType(context.Background(), type_).From(from).To(to).Domains(domains).Urls(urls).Categories(categories).Policycategories(policycategories).Ip(ip).Ports(ports).Identityids(identityids).Identitytypes(identitytypes).Applicationid(applicationid).Ruleid(ruleid).Verdict(verdict).Securityoverridden(securityoverridden).Bundleid(bundleid).Threats(threats).Threattypes(threattypes).Ampdisposition(ampdisposition).Antivirusthreats(antivirusthreats).Datalosspreventionstate(datalosspreventionstate).Filternoisydomains(filternoisydomains).Timezone(timezone).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TotalRequestsAPI.GetTotalRequestsByType``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTotalRequestsByType`: GetTotalRequests200Response
	fmt.Fprintf(os.Stdout, "Response from `TotalRequestsAPI.GetTotalRequestsByType`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetTotalRequestsByTypeRequest struct via the builder pattern
